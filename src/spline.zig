...
const Polygon = @import("path.zig").Polygon;

/// Represents a bezier curve (also referred to interchangeably here as a
/// "spline").
pub const Spline = struct {
    ...
    /// the control points. Returns null if the curve would be fully degenerate
    /// (and as such, a line should be drawn instead of a curve.
    pub fn init(
        polygon: *Polygon,
        a: Point,
        b: Point,
    ...
    /// Decomposes a spline into line approximations, which are plotted on to
    /// the spline's polygon.
    ///
    /// Takes a tolerance, aka the interpolation ratio.
    pub fn decompose(self: *Spline, tolerance: f64) !void {
        // Grab the knots of the curve (pretty much our control points at this
        // point).
        var s1 = self.knots;
    ...
    fn decomposeInto(self: *Spline, s1: *Knots, tolerance: f64) !void {
        ...
        try self.decomposeInto(s1, tolerance);
        return try self.decomposeInto(&s2, tolerance);
    }
};
